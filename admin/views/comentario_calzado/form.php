    ...
    <div class="card mx-4 mx-md-5 shadow-5-strong" style="margin-top: -100px; background: hsla(0, 0%, 100%, 0.8);backdrop-filter: blur(30px);">
        <div class="card-body py-5 px-md-5">
            <div class="row d-flex justify-content-center">
                <div class="col-lg-8">
                    <h2 class="fw-bold mb-5"><?php echo ($action == 'edit') ? 'Modificar ' : 'Nuevo ' ?>comentario de calzado</h2>
                    <form class="container-fluid" method="POST" action="comentario_calzado.php?action=<?php echo $action; ?>" enctype="multipart/form-data">
                        <!-- 2 column grid layout with text inputs for the first and last names -->
                        <div class="form-outline mb-4">
                            <textarea name="data[comentario_calzado]" class="form-control" required><?php echo isset($data[0]['comentario_calzado']) ? $data[0]['comentario_calzado'] : ''; ?></textarea>
                            <label class="form-label">Comentario de calzado</label>
                        </div>
                        ...
                        <div class="row">
                            <div class="col-md-6 mb-4">
                                ...
                                <select name="data[id_calzado]" class="form-control" required>
                                <?php
                                    foreach ($data_calzado as $key => $calzado): 
                                    $selected = " ";
                                    if ($calzado['id_calzado']==$data[0]['id_calzado']):
                                        $selected = " selected";
                                    endif;?>
                                    <option value="<?php echo $calzado['id_calzado']; ?>" <?php echo $selected; ?>><?php echo $calzado['calzado']; ?></option>
                                <?php endforeach; ?>
                                </select>
                        ...
                        <div class="mb-3">
                            <?php if ($action == 'edit'): ?>
                                <input type="hidden" name="data[id_comentario_calzado]" value="<?php echo isset($data[0]['id_comentario_calzado']) ? $data[0]['id_comentario_calzado'] : ''; ?>">
                            <?php endif; ?>
