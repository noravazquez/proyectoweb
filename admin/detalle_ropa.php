<?php require_once(__DIR__.'/controllers/sistema.php'); 
      ...
      require_once(__DIR__.'/controllers/comentario_ropa.php');
...
<body>
    ...
    <?php $idRopa = (isset($_GET['id'])) ? $_GET['id'] : null; ?>

    <br>
    <h2 style="text-align: center;">Detalle ropa</h2>
    ...
    <div class="product-detail">
        ...
        foreach ($data as $key => $producto) {
            $contenidoQR = 'Ropa: ' . $producto['ropa'] . PHP_EOL . 'Descripción: ' . $producto['descripcion'] . PHP_EOL . 'Precio: $' . $producto['precio'];
            ...
            <div class="card-detail">
                <div class="col">
                    <div class="product-image">
                        <img src="<?php echo $producto['imagen']; ?>" alt="<?php echo $producto['ropa']; ?>" class="highlight-image">
                    </div>
                </div>
                <div class="col">
                    <div class="row">
                            ...
                            <div class="product-info">
                                ...
                                <div class="details">
                                    ...
                                    <div class="detail-row">
                                        ...
                                        <span class="value"><?php echo $producto['categoria_ropa']; ?></span>
                                    </div>
                                    <div class="detail-row">
                                        <span class="label">Marca:</span>
                                        <span class="value"><?php echo $producto['marca_ropa']; ?></span>
                                    </div>
                                    <div class="detail-row">
                                        <span class="label">Talla:</span>
                                        <span class="value"><?php echo $producto['talla_ropa']; ?></span>
                                    </div>
                                    <div class="detail-row">
                                    ...
                                    <?php if ($producto['stock'] > 0) { ?>
                                        <form action="detalle_ropa.php?id=<?php echo $idRopa; ?>" method="POST">
    ...
    <br>
    <?php
        $comentarios = $ropa->getComentarios($idRopa);
        ...
        if (!empty($comentarios)) {
            ...
            echo '<p>No hay comentarios para esta prenda.</p>';
        }
    ?>
...
<br>
    ...
    <div style="display: flex; justify-content: center;">
        <form action="detalle_ropa.php?id=<?php echo $idRopa; ?>" method="POST" style="max-width: 500px;">
            <label for="comentario" style="margin-bottom: 5px;">Comentario: </label>
            <textarea id="comentario" name="data[comentario_ropa]" required style="width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 4px; margin-bottom: 10px;"></textarea>
            ...
            <input type="hidden" name="data[id_ropa]" value="<?php echo ($idRopa); ?>">
        </form>
    </div>
    <br>
    <?php 
        if (isset($_POST['enviar'])) {
            ...
            $cantidad = $comentario_ropa->newComentario($data, $idCliente);
            if ($cantidad) {
                $comentario_ropa->alert('success', 'bi bi-check-circle-fill', 'NUEVO COMENTARIO ROPA', 'El comentario de la ropa fue agregado correctamente.');
            } else {
        ...
        if (isset($_POST['agregarCarrito'])) {
            ...
            // Verificar si el ropa ya está en el carrito
            if (isset($_SESSION['carrito']['ropa'][$idRopa])) {
                // Incrementar la cantidad si ya está en el carrito
                $_SESSION['carrito']['ropa'][$idRopa]++;
            } else {
                // Agregar el ropa al carrito con cantidad 1
                $_SESSION['carrito']['ropa'][$idRopa] = 1;
            }
        }
    ?>
